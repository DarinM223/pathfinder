defmodule PathfinderWeb.AccountsTest do
  ...
  test "create_user validates password and password_confirm matches" do
    assert {:error, _} =
      Accounts.create_user(%{username: "blah", password: "secret", password_confirm: "wrong"})
  end
  ...
  test "create_user validates username is between 1 and 20 characters" do
    assert {:error, _} =
      Accounts.create_user(%{username: "", password: "secret", password_confirm: "secret"})
    assert {:error, _} =
      Accounts.create_user(%{username: "123456789012345678901",
  ...
  test "create_user validates password is between 6 and 100 characters" do
    ...
    assert {:error, _} =
      Accounts.create_user(%{username: "blah", password: "abcde", password_confirm: "abcde"})
    assert {:error, _} =
      Accounts.create_user(%{username: "blah", password: long_pass, password_confirm: long_pass})
  end

  test "create_user creates user with hash" do
    {:ok, user} =
      Accounts.create_user(%{name: "bob", username: "blah",
                             password: "secret", password_confirm: "secret"})
