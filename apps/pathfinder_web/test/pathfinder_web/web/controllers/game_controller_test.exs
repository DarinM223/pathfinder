defmodule PathfinderWeb.Web.GameControllerTest do
  ...
  @tag login_as: "bob"
  test ":create successfully creates game with existing user and redirects", %{
    conn: conn,
    user: user
  } do
    {:ok, other} = insert_user(%{username: "dave"})
  ...
  @tag login_as: "bob"
  test ":create successfully creates game with nonexisting user and redirects", %{
    conn: conn,
    user: user
  } do
  ...
  defp count_games() do
    import Ecto.Query
    PathfinderWeb.Repo.one(from(g in PathfinderWeb.Data.Game, select: count(g.id)))
  end
end
